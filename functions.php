...
if ( ! function_exists( 'edge_setup' ) ) :
	...
	function edge_setup() {
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'edge' ),
		) );
...
function edge_content_width() {
	...
	$GLOBALS['content_width'] = apply_filters( 'edge_content_width', 640 );
}
add_action( 'after_setup_theme', 'edge_content_width', 0 );
...
function edge_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'edge' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'edge' ),
...
function edge_scripts() {
	...
	wp_enqueue_script( 'edge-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
